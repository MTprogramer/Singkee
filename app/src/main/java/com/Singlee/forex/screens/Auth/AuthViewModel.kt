...
class AuthViewModel @Inject constructor(
    ...
    fun signUp(email: String, password: String, name: String) = viewModelScope.launch {
        ...
        fun resetPassword(email: String, userPass: String, newPass: String) =
            viewModelScope.launch {
                ...
                authenticationRepository.loginUser(email, userPass).collect {
                    when (it) {
                        ...
                        Response.Empty -> {}
                        is Response.Error -> {
                            _resetPassword.value = Response.Error("Something Wrong")
                        }
            ...
            authenticationRepository.loginUser(email, password).collect { result ->
                when (result) {
                    is Response.Success -> {
                        ...
                        userRepo.getUserData(userId.toString()).collect { userDataResponse ->
                            when (userDataResponse) {
                                is Response.Success -> {
                                    val userData = userDataResponse.data // Extract UserData
                                    _signInStatus.value =
                                        userDataResponse // Updating with Response<UserData>
                                    sharedPrefs.saveUser(userData) // Pass UserData to saveUser
                                }

                                is Response.Error -> {
                                    _signInStatus.value = Response.Error(userDataResponse.message)
                                }
                    ...
                    is Response.Empty -> {}
                    is Response.Error -> {
                        _signInStatus.value = Response.Error(result.message)
                    }
